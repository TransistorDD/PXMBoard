				...
				if($objUserProfile->insertData($this->m_objConfig->uniqueRegistrationMails())){	// insert profiledata >>>
					...
					if($this->m_objConfig->useDirectRegistration()){
						...
						if(@mail($objUserProfile->getPrivateMail(),
								 ...
								 str_replace(array("%password%","%username%"),array($sPassword,$objUserProfile->getUserName()),$objRegistrationMailBody->getMessage()),
					...
					if($bSuccess){
						...
						$this->m_objTemplate->addData(array("message" => eSuccessMessage::USER_REGISTERED->value));
					}
				}
				else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::USERNAME_ALREADY_EXISTS);// user already registered
			}
			else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::INVALID_EMAIL);	// invalid email
		}
	}
}
?>