	...
	public function performAction(): void{
		...
		if(!empty($sPassword1) && !empty($sPassword2)){
			if($this->getActiveUser()->changePassword($sPassword1,$sPassword2)){
				...
				$this->m_objTemplate->addData(array("message" => eSuccessMessage::USER_PASSWORD_CHANGED->value));
			}
			else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::COULD_NOT_UPDATE_PASSWORD);// pwd not valid
		}
		else{
