...
class cActionMessageeditsave extends cAction{
	...
	public function performAction(): void{
							...
							if($bAdminMode || $objBoardMessage->isThreadActive()){									
								if($bAdminMode || ($objActiveUser->getId() == $objBoardMessage->getAuthorId())){
									if($bAdminMode || $objBoardMessage->getReplyQuantity()<1){
										...
										if($bPublish){
											$objBoardMessage->setBody($sBody." ".str_replace(array("%date%","%username%"),array(date($this->m_objConfig->getDateFormat(),$this->m_objConfig->getAccessTimestamp()),$objActiveUser->getUserName()),$objTemplate->getMessage()));
										} else {
									...
									else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::MESSAGE_HAS_REPLY);// replies exist
								}
								else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::NOT_AUTHORIZED);// forbidden
							}
							else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::THREAD_CLOSED);		// thread closed
						}
						else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::INVALID_MESSAGE_ID);		// invalid msg id
					}
					else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::INVALID_MESSAGE_ID);			// invalid msg id
				}
			}
		...
		else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::BOARD_CLOSED);							// board closed
	}
}
?>