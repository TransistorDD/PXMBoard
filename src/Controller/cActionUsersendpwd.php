...
class cActionUsersendpwd extends cAction{
	...
	public function performAction(): void{
		...
		if(!empty($sUserName)){
				...
				if($objUser->getStatus() === UserStatus::ACTIVE){
					if(strcasecmp($sEmail,$objUser->getPrivateMail())==0){
						...
						if(@mail($objUser->getPrivateMail(),
								 ...
								 str_replace(array("%key%","%username%"),array($objUser->createNewPasswordKey(),$objUser->getUserName()),$sPasswordMailBody),
							...
							$this->m_objTemplate->addData(array("message" => eSuccessMessage::USER_PASSWORD_RESET_REQUESTED->value));
						}
						else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::COULD_NOT_SEND_EMAIL);// could not send email
					}
					else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::DATA_MISMATCH);	// data does not match
				}
				else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::NOT_AUTHORIZED);		// forbidden
			}
			else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::USERNAME_UNKNOWN);				// invalid username
		}
			...
			if($objUser->loadDataByPasswordKey($sPasswordKey)){
				if($objUser->getStatus() === UserStatus::ACTIVE){
					...
					if($objUser->updateData()){
						if(@mail($objUser->getPrivateMail(),
								 $sPasswordMailSubject,
								 str_replace(array("%password%","%username%"),array($sPassword,$objUser->getUserName()),$sPasswordMailBody),
								 "From: ".$this->m_objConfig->getMailWebmaster()."\nReply-To: ".$this->m_objConfig->getMailWebmaster())){

							$this->m_objTemplate = $this->_getTemplateObject("confirm");
							$this->m_objTemplate->addData($this->getContextDataArray());
							$this->m_objTemplate->addData(array("message" => eSuccessMessage::USER_PASSWORD_SENT->value));
						}
						else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::COULD_NOT_SEND_EMAIL);// could not send email
					}
			...
			else $this->m_objTemplate = $this->_getErrorTemplateObject(eError::INVALID_USER_ID);		// invalid userid
		}
		else{
