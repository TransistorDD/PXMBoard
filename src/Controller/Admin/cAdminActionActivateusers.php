					...
					if($objUser->loadDataById($iUserId)){
						...
						if($objUser->updateData()){
							...
							if(@mail($objUser->getPrivateMail(),
									 ...
									 str_replace(array("%password%","%username%"),array($sPassword,$objUser->getUserName()),$sRegistrationMailBody),
									 "From: ".$this->m_objConfig->getMailWebmaster()."\nReply-To: ".$this->m_objConfig->getMailWebmaster())){

								$this->m_sOutput .= "mail sent<br>\n";
							}
							else $this->m_sOutput .= "<span class=\"pxm-error\"> could not send mail to ".htmlspecialchars($objUser->getPrivateMail())."</span><br>\n";
						}
						else $this->m_sOutput .= "<span class=\"pxm-error\"> could not activate user</span><br>\n";
					}
					else $this->m_sOutput .= "<span class=\"pxm-error\"> invalid userid</span><br>\n";
				}
			}
				...
				if($objUser->loadDataById($iUserId)){
					if($objUser->deleteData()){
						...
						if(@mail($objUser->getPrivateMail(),
								 ...
								 str_replace(array("%username%","%reason%"),array($objUser->getUserName(),$this->m_objInputHandler->getStringFormVar("r$iUserId","notification",true,true,"rtrim")),$sRegistrationDeclineMailBody),
								  "From: ".$this->m_objConfig->getMailWebmaster()."\nReply-To: ".$this->m_objConfig->getMailWebmaster())){
					...
					else $this->m_sOutput .= "$iUserId -> <span class=\"pxm-error\">could not delete data</span><br>\n";
				}
				else $this->m_sOutput .= "$iUserId -> <span class=\"pxm-error\">user not found</span><br>\n";
			}
