...
class cActionMessagesave extends cAction{
	...
	public function performAction(): void{
				...
				if($this->m_objConfig->useQuickPost()){
					...
					if(!empty($sUserName)){
						if($objActiveUser->loadDataByUserName($sUserName)){
								...
								else if($objActiveUser->getStatus() != UserStatus::ACTIVE){
					...
					else $arrErrors[] = eError::USERNAME_REQUIRED;				// empty username
				}
				else $arrErrors[] = eError::NOT_LOGGED_IN;						// not loged in
			}
		}
		...
		if(!empty($arrErrors) || ($bEditMode)){
			...
			$this->m_objTemplate->addData($this->getContextDataArray(array("quickpost" => $this->m_objConfig->useQuickPost())));

			if(!empty($arrErrors)){
				$arrErrorTexts = array();
				foreach($arrErrors as $objError){
					$arrErrorTexts[] = array("text" => $objError->value);
				}
				$this->m_objTemplate->addData(array("error"=>$arrErrorTexts));
			}

			// parse the message body
			$objPxmParser = $this->_getPredefinedPxmParser(true);

			$this->m_objTemplate->addData(array("msg" => array("id"		=>$iMessageId,
																"subject"	=>$sSubject,
			...
			if($objActiveUser->isPostAllowed()){
				...
				if($iReturn==0){
					...
					if($objReplyMessage->loadDataById($iMessageId,$iBoardId)){
						...
						// Legacy: Send email notify_on_reply (only to original author if enabled)
								...
								if($objReplyAuthor->loadDataById($objReplyAuthor->getId()) && ($sMail = $objReplyAuthor->getPrivateMail())){
									...
									@mail($sMail,
											...
											str_replace(array("%username%",
															...
															"%boardid%",
															"%threadid%"),
														array($objActiveUser->getUserName(),
					...
					$this->m_objTemplate->addData(array(
						...
						"msg" => array(
							...
							"board" => array("id" => $objBoardMessage->getBoardId())
						)
					));
				}
				else{
					...
					$this->m_objTemplate->addData($this->getContextDataArray(array("quickpost" => $this->m_objConfig->useQuickPost())));
					$this->m_objTemplate->addData(array("error"=>array(array("text" => $error->value))));
					$this->m_objTemplate->addData(array("msg"=>array("id"		=>$iMessageId,
																				"subject"	=>$sSubject,
																				"_body"	=>$objPxmParser->parse($sBody))));
				}
