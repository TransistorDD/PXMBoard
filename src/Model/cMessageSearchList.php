...
class cMessageSearchList extends cScrollList{
	 ...
	 * @param integer $iCurrentUserId current user id (for draft visibility)
	 * @return void
	 */
	public function __construct($objSearch,$iTimeOffset,$sDateFormat,$iCurrentUserId = 0){
	 ...
	 * @param boolean $bGroupByThread group results by thread
	 * @return boolean success / failure
	 */
	public function loadData($iCurPageId,$iResultRowLimit,$bGroupByThread = false): bool{
	...
	protected function _groupResultsByThread(): void{
		...
		if($objResultSet = cDBFactory::getInstance()->executeQuery($sQuery)){
			while($objRow = $objResultSet->getNextResultRowObject()){
				$arrRootMessages[intval($objRow->m_threadid)] = array(
					"id"		=> $objRow->m_id,
