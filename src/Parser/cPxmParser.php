...
class cPxmParser extends cParser{
                    ...
                    switch ($sMessagePart[0]) { // special character handling
                        ...
                        case "[" :	// start of pxm code tag
                            ...
                            if (preg_match('/^\[([biushqm]):/iu', $sMessagePart, $arrStyleMatch)) {
                                ...
                                $iCharPointer += 2;
							// links
                            } else if (preg_match('/^\[((https?|ftps?|www|mailto:)([^\] ]+))/iu', $sMessagePart, $arrLinkMatch)) {
								if(!$bMemberSkip){
							...
							// images
                            } else if (preg_match('/^\[img:((https?[^\] ]+)\.(?:jpg|gif|png|jpeg))/iu', $sMessagePart, $arrImgMatch)) {
								if(!$bMemberSkip){
							...
							} else if (preg_match('/^\[user:(\d+)\]/iu',substr($sText,$iCharPointer), $arrMentionMatch)) {
								if(!$bMemberSkip){
							...
							} else if (preg_match('/^\[yt:([^\]]+)\]/iu',substr($sText,$iCharPointer), $arrYtMatch)) {
								if(!$bMemberSkip){
							...
							} else if (preg_match('/^\[ttv:([^\]]+)\]/iu',substr($sText,$iCharPointer), $arrTtvMatch)) {
								if(!$bMemberSkip){
	 ...
	 * @return void
	 */
	public function setReplacements($arrReplacements){
	 ...
	 *
	 * @return boolean embed external content?
	 */
	 ...
	 *
	 * @param boolean $bEmbedExternal embed external content?
	 * @return void
	 */
	 ...
	 *
	 * @param boolean $bIsLoggedIn is user logged in?
	 * @return void
	 */
	public function setIsLoggedIn($bIsLoggedIn){
	 ...
	 * @return void
	 */
	protected function _preloadMentions($sText){
	 ...
	 */
	private function _extractYouTubeId($sInput){
		$sInput = trim($sInput);
	 ...
	 */
	private function _extractTwitchData($sUrl){
		$sUrl = trim($sUrl);
