...
class cSkinTemplateXslt extends cSkinTemplate
{
    ...
    public function __construct(string $sSkinDir)
    {
        ...
        $this->m_sXmlDoc = 	 "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                            ."<!DOCTYPE pxmboard [\n<!ENTITY nbsp \"&#160;\">\n]>\n"
                            ."<pxmboard>\n";
    }
     ...
     *
     * @param array<int|string, mixed> $arrData key - value pairs
        ...
        foreach ($arrData as $mKey => $mVal) {
            ...
            if (is_array($mVal)) {
                ...
                if ($mVal === 0 || $mVal === '') {
                    ...
                    // avoid CDATA breakout
                    $this->m_sXmlDoc .= "<$mKey><![CDATA[".str_replace(']]>', ']]]]><![CDATA[>', $mVal)."]]></$mKey>\n";
                }
            }
        }
        return true;
